...
import { ICInformation, TeacherInformationType,} from "@/pages/eCard/data";
import {DeleteOutlined, EditOutlined} from "@ant-design/icons/lib";
import Model ,{StateType} from "./models/teacher"
...
interface TeacherProps extends StateType{

  dispatch: Dispatch;

}

const title: string = '教师信息配置';
const subTitle: string = '教师基础信息查询、编辑、创建、删除等操作，同时可以查询到人员名下所有IC卡的持卡信息';

class Teacher extends React.Component<TeacherProps, any> {

  constructor(props: TeacherProps, context: any) {
    super(props, context);
  }

  private columns: ColumnsType<TeacherInformationType> = [
    {
      key: 'name',
      title: '姓名',
      dataIndex: 'name',
      ...
      render: (value, record) => {
        return (
          <Space>
            ...
            <Popconfirm title="是否要删除此教师？" onConfirm={() => {
            }}>
  ...
  render() {
    ...
    const searchItem: ProFormSearchItemArrays = [
      {
        label: '姓名',
        name: 'name',
        children: <Input style={{width: '100%'}} placeholder={'请输入教师姓名'}/>, ...searchFormItemLayout
      }, {
        label: '院系', name: 'departmentName', children: <Select placeholder={'请选择所属院系'}
                                                               style={{width: '100%'}}/>, ...searchFormItemLayout
      }, {
        label: '工号', name: 'employeeID', children: <Input style={{width: '100%'}}
                                                          placeholder={'请输入教师工号'}/>, ...searchFormItemLayout,
      }, {
    ...
    return (<div >
        ...
        <div style={{textAlign: 'right', padding: '8px 16px 0 16px'}}>
          <Space><Button type={'primary'}>新建教师</Button><Button type={'primary'}>批量导入</Button></Space></div>
        <div className={'site-custom-background'} style={{marginTop: 8}}>
          <ProTable<TeacherInformationType> title={'教师'} namespace={'PeopleEditTeacher'} data={data}
